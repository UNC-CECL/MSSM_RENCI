...
% Purpose: This function creates synthetic time series from water level and
% wave data using the method of Wahl et al., 2016
%
% SEE ALSO: t_tide
%
% Record of revisions:
...
%       =========================================================
%       12/09/22        IRBReeves            Reformatted for different data source
%       
%       
%   
...
rData = load('C:\\Users\\frank\\OneDrive - University of North Carolina at Chapel Hill\\Chapter 3\\Ian_MSSM_RENCI\\MSSM_RENCI\\MSSM_RENCI\\NcoreBanks_WL_Hs_Tp.mat');
dtH   = datetime(rData.TT, 'ConvertFrom', 'datenum');
rHs   = rData.HS{site};
rTp   = rData.TP{site};  % 11 = TPD, 12 = TP
rWavD = rData.WDIR{site};
rSL   = rData.ZZ{site};
dtSL  =  dtH;

%% process the tidal data (b/c of data gaps)

actual_time = dtH(1) : hours(1) : dtH(end);
% data_nans = NaN(length(actual_time), 4);
% for t = 1 : length(actual_time)
%     if ismember(actual_time(t), dtH) == 1
%         
%         index = find(dtH == actual_time(t), 1);
%         data_nans(t, 1) = (rHs(index));
%         data_nans(t, 2) = (rTp(index));
%         data_nans(t, 3) = (rWavD(index));
%         data_nans(t, 4) = (rSL(index));
%     end
% end
rData_nans = load('C:\\Users\\frank\\OneDrive - University of North Carolina at Chapel Hill\\Chapter 3\\Ian_MSSM_RENCI\\MSSM_RENCI\\MSSM_RENCI\\NCBdata_nans.mat');
data_nans = rData_nans.data_nans;
rHs = data_nans(:, 1)';
rTp = data_nans(:, 2)';
rWavD = data_nans(:, 3)';
rSL = data_nans(:, 4)';
...
rHs_corr = rHs_rm + median(rHs(idYrs:end), 'omitnan');
rTp_corr = rTp_rm + median(rTp(idYrs:end), 'omitnan');
...
for iChunk = 1:length(cChunk)
    ...
    if  ~isempty(cChunk{iChunk})
        ...
        nYrs      = floor(days(nEnd - nStart) / 365) - 1; 
        iTideSt   = 1;
        rTideOut  = [];
...
nYrs = floor(days(dtH(end) - dtH(1))/ 365) - 1;
iStart = 1;
...
for iYear = 1 : nYrs
    ...
    iStop = iYear * 24 * 365;
    if iStop > length(rHs_corr)
        keyboard
%         iStop = length(rHs_corr);
    end
...
[iStormStart, iStormStop, dtStormStart, dtStormStop, cStormHs, ...
    ...
    cStormWavD, cStormNegSurgeDT, cStormNegSurgeNTR, dtYear, dtWeek, dtBiWeek] = deal(cell(0));
...
while t <= length(dtH)
    ...
    if rHs_corr(t) >= nHs_threshold && rNTR_nan(t) >=0  %    <----------------- NOTE: CAN COMMENT OUT SECOND HALF  OF CONDITIONAL TO ALLOW STORMS WITH NANS AND NEG SURGE
        stormStart = t;
        dur = 1;
        t = t + 1;
        ...
        if dur > dur_min
            ...
            dtWeek{end+1}  = week(dtH(stormStart));
            dtBiWeek{end+1}  = floor((week(dtH(stormStart)) + 1) / 2);
...
rWeek = cell2mat(dtWeek)';
rBiWeek = cell2mat(dtBiWeek)';
...
writematrix(SimStorms,'SeaStormMatrix.csv')
